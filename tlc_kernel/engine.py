...
class ContractWindow:
    ...
    def evaluate_telemetry_invariants(self, telemetry: Dict[str, Any]) -> None:
        ...
        if self.invariant_registry["I_8_Narrative"] == InvariantStatus.SATISFIED:
            repeat_rate = telemetry.get("repeat_rate", 0.0)
            brand_known_ratio = telemetry.get("brand_known_ratio", 0.0)
            private_label_avoidance = telemetry.get("private_label_avoidance", 0.0)
            
            if repeat_rate > 0.7 and brand_known_ratio > 0.6 and private_label_avoidance > 0.5:
        ...
        visible_premium = telemetry.get("visible_category_premium", 0.0)
        private_lift = telemetry.get("private_category_lift", 1.0)
        
        if event_spike > 2.0 and visible_premium > 1.4 and private_lift < 1.1:
        ...
        if silhouette > 0.5 and occasion_match > 0.7:
            self.invariant_registry["I_4_Identity_Signaling"] = InvariantStatus.SATISFIED
        elif telemetry.get("cultural_mismatch_detected", False) or telemetry.get("contextual_failure_rate", 0.0) > 0.6:
        ...
        elif telemetry.get("betrayal_signal_detected", False) and telemetry.get("switching_on_betrayal", 0.0) > 0.4:
            self.invariant_registry["I_5_Enacted_Fidelity"] = InvariantStatus.VIOLATED
        else:
        ...
        elif return_rate > 0.15 or telemetry.get("real_use_negative_sentiment", 0.0) > 0.5:
    ...
    def compile_and_validate(self, proposed_output_text: str) -> str:
        ...
        if self.invariant_registry["I_8_Narrative"] == InvariantStatus.AMBIGUOUS:
            gap_message = "Cannot interpret behavior safely without localized qualitative community narrative context records."
            self.repair_queue.append({"field": "I_8_Narrative", "remedy": gap_message})
            self.halt_authority_active = True
            raise HaltAuthorityException(
                message="Compilation Failure: Upstream Narrative invariant state resolved as AMBIGUOUS.",
